...
class TransformerEncoderLayerImproved(Module):
    ...
    def forward(self, src, memory2=None, src_attn_mask=None, src_key_padding_mask=None):
        src1 = self.norm1(src)
        src2 = self.self_attn(src1, src1, src1, attn_mask=src_attn_mask, key_padding_mask=src_key_padding_mask)[0]
...
class TransformerDecoderLayerGlobalImproved(Module):
    ...
    def forward(self, tgt, memory, memory2=None, tgt_attn_mask=None, tgt_key_padding_mask=None, *args, **kwargs):
        tgt1 = self.norm1(tgt)
        tgt2 = self.self_attn(tgt1, tgt1, tgt1, attn_mask=tgt_attn_mask, key_padding_mask=tgt_key_padding_mask)[0]
        tgt = tgt + self.dropout1(tgt2)
